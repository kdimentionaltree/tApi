...
class TonlibClient:
    def __init__(self,  
        ...
        self.max_parallel_requests = config['liteservers'][0].get("max_parallel_requests", 50)
    ...
    async def init_tonlib(self, cdll_path=None):
        ...
        wrapper.set_restart_hook(hook=self.reconnect, max_requests=1024)
                ...
                if "out_msgs" in t:
                    for o in t["out_msgs"]:
                        ...
                        try:
                            ...
                            o["message"] = ""
                            logger.warning(f"out_msg message decoding exception: {e}")
            except Exception as e:
    ...
    async def tryLocateTxByOutcomingMessage(self, source, destination, creation_lt):
        ...
        for shard_data in shards['shards']:
            ...
            txses = await self.getBlockTransactions(workchain, shardchain, count=40, block["seqno"], root_hash=block["root_hash"], file_hash=block["file_hash"])
            candidate = None
            count = 0
